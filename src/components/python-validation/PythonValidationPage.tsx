...
import { ArrowLeft, Terminal, FileJson, Send, FileCode2 } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { Button } from '../Button';
import { PythonProjectInfoForm } from './PythonProjectInfoForm';
import { PythonTestCaseList } from './PythonTestCaseList';
import { PythonAddTestCaseModal } from './PythonAddTestCaseModal';
import { PythonJsonPreview } from './PythonJsonPreview';
...
import type { PythonTestCase, PythonValidationConfig } from './types';

const API_URL = 'http://localhost:8080/api/v1/hands-on/generate-code';
const STORAGE_KEY = 'atom8_python_validation_last_response';

export function PythonValidationPage() {
  ...
  // Test cases
  const [testCases, setTestCases] = useState<PythonTestCase[]>([]);
  ...
  const handleSaveTestCase = (tc: PythonTestCase) => {
  ...
  const validate = (): string[] => {
    const errs: string[] = [];
    if (!skillType.trim()) errs.push('Skill type is required.');
    if (!projectName.trim()) errs.push('Project name is required.');
    if (!customInstructions.trim()) errs.push('Custom instructions is required.');
  ...
  const buildConfig = (): PythonValidationConfig => ({
    skillType,
    projectName,
    customInstructions,
    testcases: testCases,
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <header className="sticky top-0 bg-white/95 backdrop-blur-sm border-b border-slate-200 z-40 shadow-xs">
        <div className="container-tight px-4 md:px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="font-bold text-lg text-slate-900">Python Based Validations</span>
              <span className="hidden sm:inline text-sm text-slate-400 ml-2">Backend API testing</span>
            </div>
          </div>
        </div>
      </header>
        ...
        {savedResponse && !codeResponse && (
          <div className="grid grid-cols-3 gap-3">
            <button
              ...
              className="bg-emerald-50 border border-emerald-200 rounded-lg px-4 py-4 text-left hover:bg-emerald-100 transition-colors"
            >
              <div className="flex items-center gap-1.5 mb-2">
                <FileCode2 className="w-4 h-4 text-emerald-600" />
                <span className="text-sm font-bold text-emerald-700">Last Response</span>
              </div>
      ...
      <PythonAddTestCaseModal
        ...
        existingTestCaseNames={testCases.map((tc) => tc.name)}
      />

      {showJson && (
        <PythonJsonPreview config={buildConfig()} onClose={() => setShowJson(false)} />
      )}
